...
export default function BalanceCard({ baseBalance, mpesa, airtel }) {
  return (
    <div className="bg-surface p-6 rounded-xl shadow-lg">
      ...
      <p className="text-4xl font-bold mb-4">{formatCurrency(baseBalance, 'USD')} USDC</p>
      <div className="space-y-2">
        <div className="flex justify-between">
          <span>M-Pesa</span>
          <span className="font-mono">{formatKES(mpesa)} KES</span>
        </div>
        <div className="flex justify-between">
          <span>Airtel</span>
          <span className="font-mono">{formatKES(airtel)} KES</span>
        </div>
      </div>
    </div>
  )
